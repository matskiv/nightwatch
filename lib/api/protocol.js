...
var elementsByJQuery = require('./element-commands/_elementsByJQuery.js');
...
module.exports = function(Nightwatch) {
  ...
  Actions.element = function(using, value, callback) {
    ...
    if(using === 'jquery') {
      return new elementsByJQuery(Nightwatch, true).command(value, callback);
    }
  ...
  Actions.elementIdElement = function(id, using, value, callback) {
    ...
    if(using === 'jquery') {
      return new elementsByJQuery(Nightwatch, true).command(value, callback);//TODO
    }
  ...
  Actions.elements = function(using, value, callback) {
    ...
    if(using === 'jquery') {
      return new elementsByJQuery(Nightwatch, false).command(value, callback);
    }
  ...
  Actions.elementIdElements = function(id, using, value, callback) {
    ...
    if(using === 'jquery') {
      return new elementsByJQuery(Nightwatch, false).command(value, callback);//TODO
    }
   ...
   * @api protocol
   */
  Actions.elementIdDisplayed = function(using, id, selector, callback) {
    ...
    if(using === 'jquery'){
      return new elementsByJQuery(Nightwatch, true).command(selector, function (result) {
        if (result.value.ELEMENT !== undefined && result.status === 0) {
          result.value = true;
        }
        callback(result);
      });
