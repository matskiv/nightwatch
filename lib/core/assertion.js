  ...
  this.assert = function assert(passed, receivedValue, expectedValue, message, abortOnFailure, originalStackTrace) {
    ...
    if (!passed && abortOnFailure && client.options.start_session) {
      ...
      if (client.options.screenshots.onFail) {
        ...
        var fileNameStart = 'FAIL__' + client.api.currentTest.module + '.js__' + client.api.currentTest.name + '__';
        var fileNameEnd = '_' + theBrowser + '_' + dateStamp + '.png';
        var fileNameMiddle = (message ? message.replace(/[^a-z0-9]/gi, '_') : '')
        ...
        var fileName = fileNameStart + fileNameMiddle + fileNameEnd;
        if (fileName.length > 255) { // To prevent ENAMETOOLONG error
          fileName = fileNameStart + fileNameMiddle.substring(0, 255 - fileNameStart.length - fileNameEnd.length - 1) + fileNameEnd;
        }
